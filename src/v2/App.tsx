...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#F7F3EC] text-[#2F2F2F] antialiased relative overflow-hidden" id="hulu-world-app">
      ...
      {/* Sticky Header Nav */}
      <header
        className={`fixed top-0 inset-x-0 z-[5000] transition-all duration-500 backdrop-blur-md ${isScrolled
          ? 'bg-[#FAF8F4]/80 shadow-sm border-b border-[#ECE7DE]/50 py-3'
          : 'bg-[#FAF8F4]/30'
          }`}
      ...
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between" id="header-container">
          ...
          >
            <img
              src="/originals/HuluHulu Logo FA-02.png"
              alt="Hulu Hulu"
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            ...
            >
              <div className="px-6 flex flex-col gap-4" id="mobile-links-container">
                {navLinks.map((link) => (
                  <a
                    key={link.id}
                    href={`#${link.id}`}
