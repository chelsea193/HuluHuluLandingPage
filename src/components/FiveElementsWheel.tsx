...
export default function FiveElementsWheel() {
  ...
  return (
    <section className="relative py-24 px-6 md:px-12 max-w-7xl mx-auto rounded-3xl bg-[#FAF8F4] overflow-hidden shadow-sm my-16 border border-[#ECE7DE]" id="section-5">
      ...
      />
      <div className="relative z-10 text-center max-w-3xl mx-auto mb-16" id="five-elements-header">
        ...
        <h2 className="text-3xl md:text-4xl font-noto-sans-sc text-[#2F2F2F] tracking-tight leading-tight mt-3 mb-6" id="section-3-title">
          重新认识你每天吃的食物
        </h2>
              ...
              return (
                ...
                >
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="w-14 h-14 md:w-16 md:h-16 rounded-full flex flex-col items-center justify-center shadow-lg cursor-pointer font-noto-sans-sc relative"
                    style={{
                      backgroundColor: isActive ? item.color : '#F7F3EC',
                      color: isActive ? '#FFFFFF' : '#2F2F2F',
                      border: `2px solid ${item.color}`
                    }}
                  >
                    <span className="text-lg md:text-xl font-bold">{item.chineseName}</span>
                    <span className="text-[9px] uppercase tracking-wider opacity-90">{item.englishName}</span>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="flex items-center gap-4 mb-6" id="element-title-panel">
                <span
                  className="w-12 h-12 rounded-full flex items-center justify-center text-white text-xl font-bold font-noto-sans-sc"
                ...
                <div>
                  ...
                  <div className="text-sm font-noto-sans-sc tracking-wide text-gray-500 flex items-center gap-1 mt-1" id="panel-subtitle">
                    <Heart className="w-4 h-4" /> 对应腑脏：<strong className="text-gray-700">{activeElement.bodyOrgan}</strong>
                  </div>
                </div>
              </div>
      ...
      <div className="relative z-10 mt-16 bg-[#F7F3EC] p-6 md:p-8 rounded-2xl border border-[#DCD5C9] max-w-4xl mx-auto" id="elements-closing-wisdom">
        <span className="text-sm font-noto-sans-sc text-gray-600 leading-relaxed font-light text-center" id="wisdom-text">
